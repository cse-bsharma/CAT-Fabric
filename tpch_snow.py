sql_queries = ["""select /*Q1*/ count(*) from (SELECT L_RETURNFLAG
	,L_LINESTATUS
...
FROM (
	SELECT N1.N_NAME AS SUPP_NATION
		...
		,date_part(yy, L_SHIPDATE) AS L_YEAR
...
FROM (
	SELECT date_part(yy, O_ORDERDATE) AS O_YEAR
		,L_EXTENDEDPRICE * (1 - L_DISCOUNT) AS VOLUME
		,N2.N_NAME AS NATION
	FROM snowflake_sample_data.tpch_sf1000.PART
		,snowflake_sample_data.tpch_sf1000.SUPPLIER
...
"""select  /*Q9*/count(*) from (
SELECT NATION
	,O_YEAR
	,SUM(AMOUNT) AS SUM_PROFIT
FROM (
	SELECT N_NAME AS NATION
		,date_part(yy, O_ORDERDATE) AS O_YEAR
...
FROM (
	SELECT C_CUSTKEY
		,COUNT(O_ORDERKEY)
	FROM snowflake_sample_data.tpch_sf1000.CUSTOMER
	LEFT OUTER JOIN snowflake_sample_data.tpch_sf1000.ORDERS ON C_CUSTKEY = O_CUSTKEY
		AND O_COMMENT NOT LIKE '%%special%%requests%%'
	GROUP BY C_CUSTKEY
	) AS C_ORDERS(C_CUSTKEY, C_COUNT)
GROUP BY C_COUNT
)dt""",
"""select  /*Q14*/count(*) from (
